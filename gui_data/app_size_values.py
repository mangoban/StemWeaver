...
def get_screen_height():
    """Return (height, width) of primary screen.

    Tries to use screeninfo.get_monitors(); if that package isn't
    available, falls back to tkinter's winfo_screen* methods so the
    module can be imported on systems without the optional dependency.
    """
    ...
    try:
        ...
        # As a last resort, return a sensible default for headless or unknown env
        return 1080, 1920
...
class ImagePath():
    def __init__(self, base_path):
        ...
        self.banner_path = os.path.join(img_path, 'StemWeaver-banner.png')
    ...
    def open_image(self, path: str, size: tuple = None, keep_aspect: bool = True, rotate: int = 0) -> ImageTk.PhotoImage:
        ...
        # Pillow 10 removed Image.ANTIALIAS in favor of Image.Resampling
        try:
            resample_filter = Image.Resampling.LANCZOS
        except Exception:
            # older Pillow versions expose LANCZOS at module level
            resample_filter = Image.LANCZOS

        if size is not None:
            size = (int(size[0]), int(size[1]))
            if keep_aspect:
                img = img.resize((size[0], int(size[0] * ratio)), resample=resample_filter)
            else:
