...
import tkinter
try:
    # Some Python builds don't ship the tix module. Import if available.
...
def _require(tkroot):
    ...
    try:
        ...
        if platform.system() != "Linux":
            # Fallback to linux64 for unknown environments, but behaviour
            # elsewhere in the codebase assumes Linux.
...
if tix is not None:
    ...
    # Provide a compatible fallback when tix is missing: use the regular
    # Tk class. This allows importing the package on systems without tix.
    class TixTk(Tk):
        def __init__(self, *args, **kw):
            # Initialize as a normal Tk window and attempt to load tkdnd.
            super().__init__(*args, **kw)
            try:
                self.TkdndVersion = _require(self)
            except Exception:
                # If tkdnd can't be loaded, keep going; drag-and-drop may not work.
